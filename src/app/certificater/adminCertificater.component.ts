...
@Component({
  selector: 'adminCertificater',
  templateUrl: './adminCertificater.html',
})
export class AdminCertificater {
  public certificater = [];
  public base64Img = "";
  public comLogo = "";
  public TSignature = "";
  constructor(public router: Router, private translate: TranslateService, private spinner: NgxSpinnerService, protected service: CertificaterService,
    public dialog: MatDialog, public _globals: Globals) {
    // console.log(service);
    this.spinner.show();
  ...
  loadCertificater() {
    this.service.getAdminCertificateByCompany().subscribe((data) => {
      if (data.success) {

        this.certificater = data.data;
        // console.log(this.certificater);
        // this.certificater.forEach(certi => {
        //   certi.SignatureimgPath = certi.SignatureimgPath.slice(4);
        //   certi.SignatureimgPath = 'API/test.php?path=' + certi.SignatureimgPath;
        // });
      } else {
        this.certificater = [];
      }
  ...
  addCompany() {
    this.spinner.show();
    this.router.navigate(['admin/certificater/add'], { skipLocationChange: false });
  }
  startSpinner() {
    this.spinner.show();
  }
  preViewCerti(certi) {
    ...
    var obj = this;
    this.translate.get('certificate.CertCreationWait').subscribe(value => { obj._globals.spinnerText = value; });
    this.spinner.show();
  ...
  PDFCertificate(certi) {
    ...
    //     doc.setFontType("bold");
    //     doc.setFontSize(20);
    //     doc.text('TT.MM.JJJJ', nPageMiddle, 95, { align: 'center', charSpace: '0' });
    ...
    //     doc.setFontType("bold");
    //     doc.setFontSize(20);
    //     doc.text('TT.MM.JJJJ', nPageMiddle, 118, { align: 'center', charSpace: '0' });
  ...
  deleteCertificate(certi) {
    this.translate.get('dialog.DeleteCertSure').subscribe(value => {
      const dialogRef = this.dialog.open(ConfirmationBoxComponent, {
        width: '400px',
        data: { companyId: certi.certificaterId, Action: false, Mes: value },
        autoFocus: false
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.service.deleteCertificate(certi.certificaterId).subscribe((data) => {
            if (data.success) {
              this.loadCertificater();
            }
          });
